...
//유저모드 동기화 기법인 크리티컬 섹션 오브젝트를 이용해 동기화

#define NUM_THREAD 50

long long num = 0;
CRITICAL_SECTION cs; //크리티컬 섹션 관리 변수

//WINAPI 반환형 = _beginthreadex가 명시하는 호출 규약을 지키기 위해 사용한다.
unsigned WINAPI threadInc(void* arg)
{
	//for문 안에 넣어도 되지만 그러면 매우 많은 수의 CS 오브젝트 획득 및 반납이 이루어져 매우 오래걸림
	EnterCriticalSection(&cs); //크리티컬 섹션 오브젝트 획득
	for (int i = 0; i < 5000000; i++)
		num++;
	LeaveCriticalSection(&cs); //크리티컬 섹션 오브젝트 반납
	return 0;
}
...
int main()
{
	...
	InitializeCriticalSection(&cs); //크리티컬 섹션 오브젝트 초기화
	...
	DeleteCriticalSection(&cs); //크리티컬 섹션 오브젝트 소멸
